<?php /*
    Copyright © 2015  Matthias von Laer
    email: matthias-vonlaer@web.de

    All content of this website is Open Game Content; you can redistribute it
    and/or modify it under the terms of the Open Game Licence which can
    be found in libera-ludo-permisilo.php
 */ ?>

<?php require 'pagxkapo.php' ?>

<title>Traduki</title>
<h1>Traduki</h1>

<p>Tiu paĝaro estas tute nekomerca kaj estas tradukita de ludantoj por ludantoj. Se vi trovas parton kiun vi emas traduki, vi volonte povas fari tion kaj tiel helpi kompletigi la paĝaron. Ankaŭ se vi trovas mistajpojn aŭ erarojn, vi povas korekti ilin. </p>
<p>Por tio vi povas klaki al (redakti) kio troveblas en la supra navigilo por traduki rekte en la retumilo.</p>
<p>Alia maniero estas ke vi elŝutas la projekton de GitHub kie la projekto haveblas kiel git-deponejo: <a href="https://github.com/oneironauto/5futpasxo" target="_blank">https://github.com/oneironauto/5futpasxo</a>.</p>
<p>Tria eblo estas simple kontakti nin (<a href="mailto:matthias-vonlaer@web.de" target="_blank">Kontakto</a>).</p>
<p>Tradukindaj partoj ekzemple estas:</p>
<ul>
    <li>Skoloj de magiisto</li>
    <li>Sangolinioj de sorĉisto</li>
    <li>Sferoj de kleriko</li>
    <li>Sorĉlistoj de 2a ĝis 9a grado</li>
    <li>Metamagiaj talentoj kaj talentoj por krei objektojn</li>
    <li>Plia enhavo de Ekipaĵo</li>
    <li>La ĉapitroj: 
        <a href="bazregulverko/kapablecoj">Kapablecoj</a>,
        <a href="bazregulverko/pliaj_reguloj">Pliaj reguloj</a>,
        <a href="bazregulverko/batalo">Batalo</a>,
        <a href="bazregulverko/magio">Magio</a>,
        <a href="bazregulverko/prestigxklasoj">Prestiĝklasoj</a>,
        <a href="bazregulverko/gvidi_la_ludon">Gvidi la ludon</a>,
        <a href="bazregulverko/la_cxirkauxajxo">La ĉirkaŭaĵo</a>,
        <a href="bazregulverko/krei_nlr">Krei NLR</a>,
        <a href="bazregulverko/magiaj_objektoj">Magiaj objektoj</a>,
        <a href="bazregulverko/eksterordinaraj_eblecoj">Eksterord. eblecoj</a>.
    </li>
    <li>La sorĉpriskriboj</li>
    <li>Manlibro de bestoj!</li>
    <li>Aventuroj...</li>
</ul>

<?php require 'pagxpiedo.php' ?>
